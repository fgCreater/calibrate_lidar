                                                  ...
                                                  const sensor_msgs::PointCloud2::ConstPtr &in_child_cloud_msg)
{
  ...
  std::cout << "This transfor                 mation can be replicated using:" << std::endl;
  ...
  std::cout << "Corresponding transformation matrix:" << std::endl
        ...
        *output_cloud+=*in_parent_cloud;
            
              // 过滤点云：定义范围
    pcl::PointCloud<pcl::PointXYZI>::Ptr filtered_cloud_ptr(new pcl::PointCloud<pcl::PointXYZI>());
    filterPointsWithinRegion(output_cloud, 
                              filtered_cloud_ptr,
                              std::make_pair(0, -3),    // x_range      
                              std::make_pair(0, -3),    // y_range
                              std::make_pair(-2.0, 2.0), // z_range
                              true); // true to remove points outside the range

    PublishCloud(calibrated_cloud_publisher_, filtered_cloud_ptr);
  // timer end
...
void ROSMultiLidarCalibratorApp::InitializeROSIo(ros::NodeHandle &in_private_handle)
{
  ...
  std::string calibrated_points_topic_str = "/livox/points_calibratedd";

  in_private_handle.param<std::string>("points_parent_src", points_parent_topic_str, "merged_points207");
  ROS_INFO("[%s] points_parent_src: %s",__APP_NAME__, points_parent_topic_str.c_str());

  in_private_handle.param<std::string>("points_child_src", points_child_topic_str, "merged_points206");
  ...
  in_private_handle.param<double>("x", initial_x_, 0);
  in_private_handle.param<double>("y", initial_y_, 0);
  ...
  in_private_handle.param<double>("yaw", initial_yaw_, 0);
