...
{
    ros::init(argc, argv, "simple_point_cloud_merger3");
    ros::NodeHandle nh;

    // 发布合并点云的主题
    merged_cloud_pub = nh.advertise<sensor_msgs::PointCloud2>("/merged_all", 100000);

    // 订阅输入点云的主题
    ros::Subscriber points_sub = nh.subscribe("/livox/points_calibratedd", 100000, points_callback);
